// netlify/functions/get-rewrite.mjs â€” polled while the rewrite builds.
...
export default async (req) => {
  ...
  try {
    ...
    const { data } = await supabase.from("content_briefs").select("rewrite,rewrite_status").eq("id", id).single();
